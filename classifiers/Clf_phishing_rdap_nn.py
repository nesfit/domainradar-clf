"""
RDAP-based Phishing classifier using a deep neural network.
"""
...
class Clf_phishing_rdap_nn:
    """
        Class for the RDAP-based deep NN phishing classifier.
    ...
    def __init__(self, options: PipelineOptions):
        ...
        # Load the NN model
        self.model = load_model(os.path.join(options.models_dir, 'phishing_rdap_nn_model.keras'))

        # Load the scaler
        self.scaler = joblib.load(os.path.join(options.boundaries_dir, 'phishing_rdap_nn_scaler.joblib'))

        # Get the number of features expected by the model
        #self.expected_feature_size = self.model.n_features_

        # Columns that are not used in the model
        self.disqualified_columns = ["rdap_time_from_last_change"]

        # Prefix for the features
        self.feature_prefix = "rdap_"
