...
class Preprocessor:
    def __init__(self):
        ...
        self.stored["phishing"]["scaler"] = joblib.load('boundaries/scaler_benign2312misp2402.joblib')
        self.stored["phishing"]["outliers"] = joblib.load('boundaries/outliers_benign2312misp2402.joblib')
        self.stored["phishing"]["cf_model"] = joblib.load('models/decision_tree_benign2312phishing2402.joblib')
        ...
        self.stored["malware"]["cf_model"] = joblib.load('models/malware_decision_tree_model.joblib')
    ...
    def apply_scaling(self, df: pd.DataFrame, classifier_type: str):
        ...
        scaled_data = self.stored[classifier_type]["scaler"].transform(numeric_df[fitted_columns])
    ...
    def adjust_outliers(self, features, classifier_type: str):
        # Apply boundaries
        for column, (lower_bound, upper_bound) in self.stored[classifier_type]["outliers"].items():
            if column in features.columns:  # Ensure the column exists in the current dataset
                features[column].apply(lambda x: x if lower_bound <= x <= upper_bound else lower_bound if x < lower_bound else upper_bound)
        return features
