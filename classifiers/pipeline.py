...
class Pipeline:
    ...
    def calculate_badness_probability(self, domain_stats: pd.Series) -> float:
        ...
        # Heuristics
        if not (
            domain_stats["phishing_avg"] > 0.8 and domain_stats["malware_avg"] > 0.8
        ):
        ...
        if not (
            ...
            domain_stats["malware_avg"] > 0.5
            and domain_stats["dga_binary_nn_result"] > 0.5
        ):
            badness_probability -= 0.1
    ...
    def generate_result(self, stats: pd.Series) -> dict:
        ...
        if stats["dga_binary_nn_result"] < 0.1:
            ...
            stats["dga_binary_nn_result"] >= 0.1 and stats["dga_binary_nn_result"] < 0.5
        ):
            dga_desc = "The domain has some similarities to DGA domains."
        elif (
            stats["dga_binary_nn_result"] >= 0.5 and stats["dga_binary_nn_result"] < 0.9
        ):
        ...
        result = {
            ...
            "classification_results": [
                ...
                },
                {
                    ...
                    "details": {
                        "CNN malware classifier": str(
                            round(stats["malware_cnn_result"] * 100, 2)
                        )
                        + "%",
    ...
    def generate_preliminary_results(
        ...
        stats["malware_sum"] = (
            stats["malware_cnn_result"]
            + stats["malware_lgbm_result"]
            + stats["malware_xgboost_result"]
        )
        ...
        stats["total_sum"] = (
            stats["phishing_sum"] + stats["malware_sum"] + stats["dga_binary_nn_result"]
        )
        stats["total_avg"] = stats["total_sum"] / (
            no_phishing_classifiers + no_malware_classifiers + 1
        )
        stats["total_prod"] = (
            stats["phishing_prod"]
            * stats["malware_prod"]
            * stats["dga_binary_nn_result"]
        )
