...
class Pipeline:
            ...
            "classification_results": [
                {
                    ...
                    "details": {
                        "CNN phishing classifier": str(
                            round(stats["phishing_cnn_result"] * 100, 2)
                        )
                        + "%",
                        "LightGBM phishing classifier": str(
                            round(stats["phishing_lgbm_result"] * 100, 2)
                        )
                        + "%",
                        "XGBoost phishing classifier": str(
                            round(stats["phishing_xgboost_result"] * 100, 2)
                        )
                        + "%",
                        "Deep NN phishing classifier": str(
                            round(stats["phishing_deepnn_result"] * 100, 2)
                        )
                        + "%",
                        "DNS-based NN phishing classifier": str(
                            round(stats["phishing_dns_nn_result"] * 100, 2)
                        )
                        + "%",
                        "RDAP-based NN phishing classifier": str(
                            round(stats["phishing_rdap_nn_result"] * 100, 2)
                        )
                        + "%",
                        "Geolocation NN phishing classifier": str(
                            round(stats["phishing_geo_nn_result"] * 100, 2)
                        )
                        + "%",
                        "IP-based NN phishing classifier": str(
                            round(stats["phishing_ip_nn_result"] * 100, 2)
                        )
                        + "%",
                    },
                },
                {
                    ...
                    "details": {
                        ...
                        "LightGBM malware classifier": str(
                            round(stats["malware_xgboost_result"] * 100, 2)
                        )
                        + "%",
                        "XGBoost malware classifier": str(
                            round(stats["malware_xgboost_result"] * 100, 2)
                        )
                        + "%",
                        "Deep NN malware classifier": str(
                            round(stats["malware_deepnn_result"] * 100, 2)
                        )
                        + "%",
                        "DNS-based NN malware classifier": str(
                            round(stats["malware_dns_nn_result"] * 100, 2)
                        )
                        + "%",
                        "RDAP-based NN malware classifier": str(
                            round(stats["malware_rdap_nn_result"] * 100, 2)
                        )
                        + "%",
                        "Geolocation NN malware classifier": str(
                            round(stats["malware_geo_nn_result"] * 100, 2)
                        )
                        + "%",
                        "IP-based NN malware classifier": str(
                            round(stats["malware_ip_nn_result"] * 100, 2)
                        )
                        + "%",
                    },
                },
                {
                    ...
                    "details": {
                        **{
                            "_DGA binary Deep NN classifier": str(
                                round(stats["dga_binary_deepnn_result"] * 100, 2)
                            )
                            + "%",
                            "_DGA binary LightGBM classifier": str(
                                round(stats["dga_binary_lgbm_result"] * 100, 2)
                            )
                            + "%",
                        },
                        **dga_family_details,
    ...
    def run_classifiers(self, df: pd.DataFrame, stats: pd.DataFrame):
        ...
        stats["phishing_sum"] = (
            stats["phishing_cnn_result"]
            + stats["phishing_lgbm_result"]
            + stats["phishing_xgboost_result"]
            + stats["phishing_deepnn_result"]
            + stats["phishing_dns_nn_result"]
            + stats["phishing_rdap_nn_result"]
        )
        stats["phishing_avg"] = stats["phishing_sum"] / no_phishing_classifiers
